...
def _build_text_result(result: ModalityResult) -> TextResult:
    ...
    details = result.details or {}
    perplexity_features = details.get("perplexity_features", {})
    burstiness_features = details.get("burstiness_features", {})
    if not isinstance(perplexity_features, dict):
        perplexity_features = {}
    if not isinstance(burstiness_features, dict):
        burstiness_features = {}
    
    return TextResult(
        ai_probability=details.get("ai_probability", result.score),
        perplexity_score=details.get(
            "perplexity_score",
            perplexity_features.get("mean_perplexity", details.get("perplexity", 0.0))
        ),
        burstiness_score=details.get(
            "burstiness_score",
            burstiness_features.get("burstiness_score", details.get("burstiness", 0.0))
        ),
...
def _build_image_result(result: ModalityResult) -> ImageResult:
    ...
    return ImageResult(
        ...
        ensemble_score=details.get("ensemble_score", 0.0),
